...
Page({
  ...
  onLoad: function(options) {
    ...
    let buy = options.buy = options.buy.substring(0, options.buy.lastIndexOf(',')).split(",");//处理已买日期字符串

    let dates = this.initDate(mydates,buy);//初始化日期信息
   ...
   * 初始化日期信息
   */
  initDate:function(mydates,buy){
    let dates = [];
    let select = false;
    for(let i= 0;i<mydates.length;i++){
      let date = mydates[i];
      let ifhas = buy.indexOf(date);
      let obj = {};
      ...
      if (!select && obj.gone == 2 && ifhas ==-1){//如果还没有默认选择
        obj.selected = true;
        select = true;
        this.setData({
          dateIndex:i
  ...
  getCssBydate:function(obj){
    ...
    } else if (obj.gone == 5){//已被选过
      obj.style = "border:1rpx solid rgb(253, 14, 14);color:rgb(253, 14, 14);background-repeat:no-repeat;"
      obj.show = true
    }else {
  ...
  next: function() {
    ...
    app.post(API_URL, "action=BmHuoDdong&loginrandom=" + loginrandom + "&zcode=" + zcode + "&h_id=" + options.h_id + "&hddate=" + hddate + "&hdtime=" + options.hdtime + "&mobile=" + mobile + "&tname=" + tname + "&beizhu=" + beizhu + "&taocan=" + taocan, true, false, "报名中", "", "", self).then(res => {
      console.log(res)
      let b_id = res.data.data[0].bmid;
      console.log("action=BmHuoDDongPay&loginrandom=" + loginrandom + "&zcode=" + zcode + "&h_id=" + options.h_id + "&b_id=" + b_id)
      app.post(API_URL, "action=BmHuoDDongPay&loginrandom=" + loginrandom + "&zcode=" + zcode + "&h_id=" + options.h_id + "&b_id=" + b_id,false,false,"","","",self).then(res=>{
        console.log(res)
        wx.showModal({
          title: '提示',
